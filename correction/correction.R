...
assign(".CORRECTION_BOOTSTRAPPED", TRUE, envir = .GlobalEnv)
...
ensure_raw_baseline(
  base_M = base_M,
  base_form = base_form,
  matrix_path = file.path(output_folder, "raw.csv"),
  force = FALSE
)
...
REQUESTED_METHODS <- character(0)
COMPLETED_METHODS <- character(0)
CURRENT_METHOD_CODE <- NA_character_

input_checks_flag <- file.path(output_folder, ".input_checks_done")

run_input_checks <- function() {
  if (file.exists(input_checks_flag)) {
    say("ℹ️ Input checks already completed earlier; skipping.")
    return(invisible(NULL))
  }
  run_method("Input checks", {
    targets <- REQUESTED_METHODS
    if (!length(targets) && !is.na(CURRENT_METHOD_CODE)) {
      targets <- CURRENT_METHOD_CODE
    }
    ...
    if (!length(targets)) targets <- "(unknown)"
    say("Method list: ", paste(targets, collapse=", "))
    ...
    try(file.create(input_checks_flag), silent = TRUE)
  })
}

prepare_method <- function(method_code) {
  method_code <- canonical_method_code(method_code)
  if (is.na(method_code)) {
    fail_step("Prepare", "Received empty method code.")
  }
  if (!length(REQUESTED_METHODS)) {
    REQUESTED_METHODS <<- method_code
  } else if (!(method_code %in% REQUESTED_METHODS)) {
    REQUESTED_METHODS <<- c(REQUESTED_METHODS, method_code)
  }
  CURRENT_METHOD_CODE <<- method_code
  run_input_checks()
}
...
canonical_method_code <- function(code) {
  if (is.null(code) || !nzchar(as.character(code))) return(NA_character_)
  code_chr <- as.character(code)
  known <- names(expected_outputs)
  if (!length(known)) return(code_chr)
  direct <- which(code_chr == known)
  if (length(direct)) return(known[[direct[[1]]]])
  lc <- tolower(code_chr)
  for (cand in known) {
    if (tolower(cand) == lc) return(cand)
  }
  code_chr
}
...
audit_flag <- file.path(output_folder, ".output_audit_done")

finalize_method <- function() {
  if (is.na(CURRENT_METHOD_CODE) || !nzchar(CURRENT_METHOD_CODE)) {
    return(invisible(NULL))
  }
  say("🎉 Method completed: ", CURRENT_METHOD_CODE)
  COMPLETED_METHODS <<- unique(c(COMPLETED_METHODS, CURRENT_METHOD_CODE))
  targets <- REQUESTED_METHODS
  if (!length(targets)) {
    targets <- COMPLETED_METHODS
  }
  remaining <- setdiff(targets, COMPLETED_METHODS)
  if (length(remaining)) {
    say("ℹ️ Output audit deferred; remaining methods: ", paste(remaining, collapse=", "))
    return(invisible(NULL))
  }
  if (file.exists(audit_flag)) {
    say("ℹ️ Output audit already completed earlier; skipping.")
    return(invisible(NULL))
  }
  audit_outputs(targets, output_folder)
  try(file.create(audit_flag), silent = TRUE)
  invisible(NULL)
}

# ---------------------------
# Dispatcher entry point (invoked via `Rscript correction/correction.R ...`)
# ---------------------------

if (length(args) >= 2) {
  method_flag_idx <- match("--methods", args, nomatch = 0)
  if (method_flag_idx > 0) {
    requested <- args[(method_flag_idx + 1):length(args)]
    requested <- requested[nzchar(requested)]
    if (!length(requested)) {
      fail_step("Dispatcher", "No methods provided after --methods flag.")
    }
    canonical <- vapply(requested, canonical_method_code, character(1), USE.NAMES = FALSE)
    canonical <- canonical[nzchar(canonical)]
    if (!length(canonical)) {
      fail_step("Dispatcher", "Unable to resolve method codes provided to dispatcher.")
    }
    REQUESTED_METHODS <<- unique(canonical)
    say("🚀 Correction dispatcher queued methods: ", paste(REQUESTED_METHODS, collapse=", "))
    assign(".CORRECTION_DISPATCH_ACTIVE", TRUE, envir = .GlobalEnv)
    on.exit({
      if (exists(".CORRECTION_DISPATCH_ACTIVE", envir = .GlobalEnv, inherits = FALSE)) {
        rm(".CORRECTION_DISPATCH_ACTIVE", envir = .GlobalEnv)
      }
    }, add = TRUE)
    for (method_code in REQUESTED_METHODS) {
      script_name <- paste0(method_code, ".R")
      script_path <- file.path("correction", "methods", script_name)
      if (!file.exists(script_path)) {
        fail_step("Dispatcher", paste0("Method script not found: ", script_name))
      }
      say("➡️ Running method script: ", method_code)
      method_env <- new.env(parent = .GlobalEnv)
      runner <- NULL
      tryCatch(
        {
          sys.source(script_path, envir = method_env, keep.source = FALSE)
          if (exists("run_correction_method", envir = method_env, inherits = FALSE)) {
            runner <- get("run_correction_method", envir = method_env, inherits = FALSE)
          }
        },
        error = function(e) {
          fail_step(paste0("Method ", method_code), conditionMessage(e))
        }
      )
      if (!is.function(runner)) {
        fail_step(
          "Dispatcher",
          paste0("Method script did not expose run_correction_method(): ", script_name)
        )
      }
      tryCatch(
        {
    ...
    say("✅ Correction dispatcher finished all methods.")
    if (!interactive()) {
