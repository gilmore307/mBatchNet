...
run_correction_method <- function() {
  ...
  run_method(METHOD_LABEL, {
    ...
    Y <- t(get_input_for(METHOD_CODE, base_M, base_form))
    ...
    if (is.null(rownames(metadata)))
      fail_step(METHOD_CODE, "metadata rownames must match colnames (currently NULL).")
    ...
    if (!("batch_id" %in% colnames(metadata)))
      fail_step(METHOD_CODE, "metadata must contain 'batch_id' column.")
    ...
    if (nlevels(batch_factor) < 1L)
      fail_step(METHOD_CODE, "No valid batch levels after cleaning.")

    keep_rows <- rowSums(Y, na.rm = TRUE) > 0L
    if (!isTRUE(any(keep_rows)))
      fail_step(METHOD_CODE, "All genes have zero or non-finite counts after cleaning.")
    ...
    if (any(!is.finite(lib) | lib <= 0L)) {
      ...
      if (nlevels(batch_factor) < 1L)
        fail_step(METHOD_CODE, "No valid batch levels after zero-library drop.")
    }
    ...
    write_tss_clr(METHOD_LABEL, out_counts, "counts", "normalized_ruv.csv")
  })
