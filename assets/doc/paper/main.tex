...
Users can run one or more batch-effect correction methods while specifying method-specific parameters. \rev{At a high level, the supported methods cover regression and quantile-regression adjustment, empirical-Bayes and linear-model adjustment, supervised multivariate correction, and recent processing-bias or dictionary-learning approaches. Their assumptions, required inputs and adjustable parameters differ, and mBatchNet exposes method-specific settings through a common web interface.}


The assessment module applies the same metrics before and after correction, allowing direct method comparison under a common framework. For compositional analyses, abundances are transformed using the centered log-ratio \rev{(CLR)} transformation and Euclidean distance on CLR-transformed profiles is used as the Aitchison distance; Bray--Curtis analyses operate on relative abundances. Diagnostics include PCA, PCoA and NMDS in Aitchison and Bray--Curtis spaces, ANOSIM \citep{Clarke1993ANOSIM} and PERMANOVA \citep{Anderson2001PERMANOVA}, feature-wise ANOVA, pRDA, PVCA and neighborhood-based metrics such as alignment, entropy of batch mixing and silhouette summaries. \rev{Ordination and distance-based tests assess whether samples remain separated by batch; ANOVA and variance-partitioning summaries quantify variation associated with batch, target and their overlap; and neighborhood metrics summarize local cross-batch mixing and target separation.} The server also provides an example dataset and contextual help to facilitate rapid trial runs and interpretation.
