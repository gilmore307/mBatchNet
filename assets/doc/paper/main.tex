...
Pre-correction diagnostics showed clear batch-associated structure (Figure~\ref{fig1}). In Aitchison ordinations, the uncorrected data clustered by processing date (Figure~\ref{fig1}A), and Bray--Curtis distance-based analyses indicated strong batch association (Figure~\ref{fig1}B; ANOSIM $R=0.637$; PERMANOVA $R^2=0.401$). After correction, mBatchNet revealed clear method-dependent differences. ConQuR, MMUPHin, DEBIAS-M, and MetaDICT increased overlap across batches while maintaining phenotype separation, whereas PLSDA-batch preserved phenotype-associated structure but retained more visible residual batch separation (Figure~\ref{fig1}A,B; Supplementary Figures~S2--S6). Variance-partitioning analyses, including feature-wise ANOVA $R^2$ (Figure~\ref{fig1}C), pRDA (Figure~\ref{fig1}D), and PVCA (Supplementary Figure~S7), together with mixing and separation metrics (Supplementary Figure~S8), were consistent with these patterns. On this dataset, ConQuR showed the strongest attenuation of batch-associated structure while preserving phenotype separation. All generated figures, corrected tables, and run outputs can be downloaded directly from the server for downstream analysis and reporting.
